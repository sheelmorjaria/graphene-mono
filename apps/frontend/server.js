...
const server = createServer((req, res) => {
  ...
  // Sitemap: prefer the build-time dist/sitemap.xml (generated by the
  // prerender crawl — fresh on every deploy). Fall back to proxying a
  // backend-generated sitemap if the file is somehow missing.
  if (req.url === '/sitemap.xml' || req.url === '/sitemap.xml/') {
    ...
    if (fileExists(localSitemap)) {
      ...
      res.end(readFileSync(localSitemap));
      return;
    }
