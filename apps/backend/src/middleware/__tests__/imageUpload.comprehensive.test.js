...
describe('Image Upload Middleware - Comprehensive Tests', () => {
  ...
  describe('File Filter Function', () => {
    ...
    beforeEach(() => {
      ...
      if (multer.lastConfig && multer.lastConfig.fileFilter) {
        fileFilter = multer.lastConfig.fileFilter;
      } else {
        // Create a mock fileFilter based on the actual implementation
  ...
  describe('processProductImages Middleware', () => {
    ...
    it('should handle sharp initialization errors', async () => {
      // Mock sharp to throw error on first call (for first file)
      ...
      // The middleware continues with other images when one fails
      // So it should call next() instead of sending an error response
      expect(next).toHaveBeenCalledWith();
  ...
  describe('deleteProductImages Function', () => {
    ...
    it('should continue deleting other files if one fails', async () => {
      ...
      // Should still attempt to delete other files (3 successful + 1 failed = 4 total)
  ...
  describe('Multer Configuration', () => {
    it('should have multer function available', () => {
      expect(multer).toBeDefined();
      expect(typeof multer).toBe('function');
    });

    it('should have memory storage available', () => {
